...
$(function(){
	...
	$("#first_menu_type").combobox({  
		onSelect:function(item){
			var menu_type=item[0];
			if(menu_type=='view'){
				$("#first_url").textbox({required:true})
			}else{
				$("#first_url").textbox({required:false})
			}
			if(menu_type=='media_id'||menu_type=='view_limited'){
				$("#first_media_id").textbox({required:true})
			}else{
				$("#first_media_id").textbox({required:false})
			}
	...
	<div class="easyui-layout" data-options="fit:true">
		<div data-options="region:'center',border:false" style="padding: 5px;">
			<form id="addFirstMenuForm" action="${ctx }/system/wechatMenu/saveWechatMenu.jhtml"
				method="post" >
				...
				<tr>
					...
					<tr>
						...
						<td><input name="menu_type" id="first_menu_type" type="text" value="click"  editable="false" 
		...
		<div data-options="region:'south',border:false" height="35px"
			...
			<a class="easyui-linkbutton" data-options="iconCls:'ok'"href="javascript:void(0)"
			   onclick="submitFormData('addFirstMenuForm','menuList','addFirstMenuWindow','1')" style="width: 70px">保存</a> &nbsp;
		    <a class="easyui-linkbutton" data-options="iconCls:'close'" href="javascript:void(0)"
				onclick="closeWindow('addFirstMenuWindow')" style="width: 70px">关闭</a>
		</div>
	</div>
