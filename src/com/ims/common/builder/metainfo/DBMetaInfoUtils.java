package com.ims.common.builder.metainfo;
...
import com.ims.common.core.asset.IMSListUtils;
...
public class DBMetaInfoUtils {
	...
	public static List<TableVO> listTableVOs(DatabaseMetaData databaseMetaData, String likeTableName) {
		...
		if (IMSUtils.isNotEmpty(likeTableName)) {
			String jql = "SELECT * FROM :IMSList WHERE name LIKE :name";
			Dto dto = Dtos.newDto("name", "%" + likeTableName + "%");
			tableVOs = IMSListUtils.select(tableVOs, TableVO.class, jql, dto);
		}
	...
	public static TableVO getTableVO(DatabaseMetaData databaseMetaData, String tableName) {
		...
		String jql = "SELECT * FROM :IMSList WHERE name = :name";
		Dto dto = Dtos.newDto("name", tableName);
		tableVOs = IMSListUtils.select(tableVOs, TableVO.class, jql, dto);
		TableVO tableVO = null;
		if (IMSUtils.isNotEmpty(tableVOs)) {
			tableVO = tableVOs.get(0);
		}
		if (IMSUtils.isEmpty(tableVO.getComment()) || StringUtils.startsWithIgnoreCase(tableVO.getComment(), "InnoDB")) {
	...
	public static List<ColumnVO> listColumnVOs(DatabaseMetaData databaseMetaData, String equalTableName) {
		...
		for (ColumnVO columnVO : columnVOs) {
			...
			tempList = IMSListUtils.select(pkColumnVOs, ColumnVO.class, jql, dto);
			if (IMSUtils.isNotEmpty(tempList)) {
	...
	public  static List<ColumnVO> getPKColumnVOs(List<ColumnVO> columnVOs){
		String jql = "SELECT * FROM :IMSList WHERE isPkey = :isPkey";
		Dto dto = Dtos.newDto("isPkey", true);
		columnVOs = IMSListUtils.select(columnVOs, ColumnVO.class, jql, dto);
		return columnVOs;
	}
	...
	public static ColumnVO getColumnVO(DatabaseMetaData databaseMetaData, String tableName, String columnName) {
		...
		String jql = "SELECT * FROM :IMSList WHERE tablename = :tablename AND name = :name";
		Dto dto = Dtos.newDto("tablename", tableName);
		dto.put("name", columnName);
		columnVOs = IMSListUtils.select(columnVOs, ColumnVO.class, jql, dto);
		ColumnVO columnVO = null;
		if (IMSUtils.isNotEmpty(columnVOs)) {
