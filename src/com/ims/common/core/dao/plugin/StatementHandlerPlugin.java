...
import com.ims.common.core.dao.plugin.dialect.DefaultDialect;
import com.ims.common.core.dao.plugin.dialect.Dialect;
import com.ims.common.core.dao.plugin.dialect.OracleDialect;
import com.ims.common.core.dao.plugin.dialect.SqlServerDialect;
import com.ims.common.core.exception.IMSException;
import com.ims.common.core.matatype.Dto;
/**
 * 
 * 类描述：  * <b>StatementHandlerPlugin插件</b>
 * <p>
 * 1、实现分页。2、实现动态排序。
 * </p>
 * 创建人：陈骑元
 * 创建时间：2016-6-5 下午09:42:45
 * 修改人：蓝枫 
 * 修改时间：2016-6-5 下午09:42:45
 * 修改备注： 
 * @version
 */
...
public class StatementHandlerPlugin implements Interceptor {
	...
	public Object intercept( Invocation pInvocation) throws Throwable {
		...
		if (update) {
			...
			if (StringUtils.equals(IMSCons.JDBC_EXECUTE_ONLYREAD, IMSCxt.getParamValue("run_mode"))) {
				throw new IMSException(13);
			}
		}		*/
		//从现在开始只剩下Select语句了
		...
		//处理排序
		// 参数对象不为空且为Dto
		if (IMSUtils.isNotEmpty(parameterObject) && parameterObject instanceof Dto) {
			Dto paramDto = (Dto) parameterObject;
			//参数对象含有排序参数，则进行排序运算
			String orderString = paramDto.getOrder();
			if (IMSUtils.isNotEmpty(orderString)) {
				//不需要方言，排序语法都是通用的
		...
		if (parameterObject == null) {
			throw new IMSException("分页查询参数对象不能为空");
		}
		Dto paramDto = null;
		// 参数对象为非Dto实例时
		if (!(parameterObject instanceof Dto)) {
			throw new IMSException("分页查询只能以Dto作为查询参数对象");
		}
		paramDto = (Dto) parameterObject;
		Integer startInteger = paramDto.getPageStart();
		Integer limitInteger = paramDto.getPageLimit();
		if (IMSUtils.isEmpty(startInteger) || IMSUtils.isEmpty(limitInteger)) {
			throw new IMSException("分页查询参数[start,limit]不能为空");
		}
